...
% _type = 'axis' produces the legend along the x-axis while legend_type = 'plot' produces the standard legend.  See figure for more details
%
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
    ...
    % For whatever reason, handles.bars(i).XOffset only works when you
    % first display handles.bars(i) like this once.  Otherwise, it only
    % returns 0.  See discussion at https://www.mathworks.com/matlabcentral/answers/243870-matlab-2015-bug-x-position-of-bars-in-bar-plot
    handles.bars(1)
    
    % Plot errors (updated for Matlab 2014b and later)
    for i = 1:numbars
