...
if isempty(find(barvals(end, :)))
   barvals(end, :) = [];
end

errvals = cell2mat(get(h.errors, 'Ldata'))'; 
if isempty(find(errvals(end, :)))
   errvals(end, :) = [];
end

posX = cell2mat(get(h.errors, 'Xdata')');
% Fixed for Matlab 2015a for within groups brackets--not tested on between
% groups brackets.
posX = posX(1:2:end);
...
for j = 1:nGroups
    ...
    for i = sortorder %draw the brackets from lowest to highest
        ...
        plot([bX(1) bX(1) bX(2) bX(2)],...
                 [bY bY+bHeight bY+bHeight bY], 'LineWidth', 2, 'Color', [0.3 0.3 0.3]);
    end
end
...
for i = sortorder %draw the brackets from lowest to highest
    ...
    iColor = int16(max([1 (j-1)*size(color, 1)/(nSubGroups-1)]));
